...
> 约定：用户连续提问时，若当轮无法一次做完，**必须**写入本表；用户再说「优化 / 继续优化」时，按表从上到下清掉，不要丢项。
...
| 🟢 | data-safe-forex | 外管局月末外汇储备规模 | ✅ v1.121.110–114 | 通稿+官方储备 XLSX；约 45 期（含 2022-11 补缺）；≠房价 |
| 🟢 | data-safe-settle | 外管局银行结售汇月度 | ✅ v1.121.111–112 | 结售汇+涉外收付款；顺差派生；≠房价 |
| 🟢 | data-safe-fx-market | 外管局外汇市场交易概况 | ✅ v1.121.113 | 月度总成交/即期/衍生品；≠房价 |
| 🟢 | data-safe-usd-mid | 外管局汇率中间价日度 | ✅ v1.121.114–116 | RMBQuery；美元+欧元/港元/日元/英镑；≠房价 |
| 🟢 | data-safe-ora | 外管局官方储备资产分项 | ✅ v1.121.118 | 外储/IMF/SDR/黄金/合计；≠房价 |
| 🟢 | data-safe-bop-trade | 国际收支货物和服务贸易月度 | ✅ v1.121.119 | 货/服顺差；初步数；≠房价 |
| 🟢 | data-safe-iip | 国际投资头寸季报 | ✅ v1.121.120 | 资产/负债/净资产+分项；≠房价 |
| 🟢 | data-safe-bop | 国际收支平衡表季报 | ✅ v1.121.121 | 经常账户+资本金融；正式优先；≠房价 |
| 🟢 | data-nbs-trade | 海关货物进出口（经 NBS） | ✅ v1.121.122 | 海关官网 WAF；NBS 国民经济通稿；≠房价 |
| 🟢 | data-nbs-pmi | 采购经理指数（含建筑业） | ✅ v1.121.123 | 临界点 50；建筑业景气 ≠房价 |
| 🟢 | data-chinabond-yield | 中债国债收益率 | ✅ v1.121.124 | 展示表 HTML 覆盖最新日；API 回填；≠房价 |
...
| 🔵 | refactor-overview-budget | 总览长度预算 + 门禁 | ✅ v1.121.144 | 详见 [docs/DASHBOARD_OVERVIEW_BUDGET.md](../docs/DASHBOARD_OVERVIEW_BUDGET.md)；硬规则：overview macro-card ≤ 4（实际 1 张 nbs-macro）、dashboard 总行数 ≤ 11.5k（实际 10 975 → 现 11 034）；`dashboardOverviewBudget.test.ts` 4/4 通过；远期目标 ≤ 2 宏卡 + ≤ 10k 行；§7 用户待拍板 4 项（卡片数 4/8/12、市场风向 Compact 6 格、§5 迁移、macro-rates 命名）状态：作者已收口到 ≤ 4 / 0 / 0 / 命名已实施 |
...
1. **同一轮能做完的优化尽量一次做完**；做不完写入上表，状态标 ⏳。  
2. 用户说「优化 / 继续 / 下一步」→ **优先清空本「活跃待优化」表**，再开新需求。  
3. 真机才能验的项（如 App 深链）完成后把状态改为 ✅，并在备注写验证结论。
...
## I 段补 2：trend-24 装修 × 楼龄 溢价分析 (v0.44.0)

| 等级 | 编号 | 任务 | 状态 | 备注 |
|------|------|------|------|------|
| 🟢 AC | trend-24 | compute_decorate_age.py 写 48 行 | ✅ v0.44.0 | 6 楼龄段(≤1999/2000-04/.../2020+) |
| 🟢 AC | trend-24 | types + parse + store + queries (矩阵 + 豪装+毛坯行) | ✅ v0.44.0 | 6 单测 |
