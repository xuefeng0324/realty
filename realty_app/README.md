...
| 版本 | 发布日期 | 说明 |
|------|----------|------|
| v1.122.21 | 2026-09-10 | `scripts/check_csv_freshness.py` `--exempt-from-stale` 真 bug 修复：v1.122.16 加的白名单从来没真生效——白名单存的是不带 `.csv` 后缀的文件名（如 `gz_provident_annual`），但代码用 `csv_path.name`（带 `.csv` 后缀）做 `in` 比较，**永远 mismatch**。CI workflow `check-csv-freshness.yml` 自 v1.122.16 起每次跑都把10 个 EXEMPT 源当 STALE 报 → 邮件噪音持续几周。修法：构造 `exempt_set` 时如果没 `.csv` 后缀就自动补全；接受"foo.csv"/"foo"/".csv"三种写法。修后本机实测 10 个 EXEMPT 源全部正确显示 `⏸ EXEMPT`，不计入 worst 退出码；同时披露真正 STALE 的 4 个源（gd_construction 134 天 / gz_housing_plan 99 天 / sz_land_deals 78 天 / sz_planned_supply 66 天），下一轮手动补数据（versionCode 300） |
