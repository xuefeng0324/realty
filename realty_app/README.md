...
| 版本 | 发布日期 | 说明 |
|------|----------|------|
| v1.122.22 | 2026-09-10 | 5 个 UNKNOWN CSV 全部加 `publish_date` 字段识别 + 1 个补最新数据：`zh_affordable_progress.csv` 重跑爬虫从 9 行扩到 12 行（补 2026-07/08 两月数据，最新 `2026-08-28`，freshness 从 STALE 72 天 → OK 13 天）；`gz_affordable_projects/targets.csv` + `sz_affordable_projects.csv` + `zh_bdc_registration_district.csv` 用脚本加 `publish_date` 列（基于 `year + as_of_month` 或 `year + quarter` 推断为月底+半月日期），`zh_bdc_registration_district` 从 UNKNOWN → OK 27 天；3 个年度源（gz/sz affordable projects/targets）**未**扩 cron workflow EXEMPT（待用户拍板——避免擅自扩大邮件噪音范围）；`check_csv_freshness.py` `DATE_FIELD_CANDIDATES` 加 `report_date` 字段（适配 zh_affordable_progress）；5 个 UNKNOWN → 0，真 STALE 减到 4 个（与 FRESHNESS_CHECK.md v1.122.22 调研一致）。check.ps1 smoke 修端口 + EXEMPT 后缀 bug 已经分别 commit 在 v1.122.20/v1.122.21（versionCode 301） |
