...
| v1.122.51 | 2026-09-12 | CSV freshness cron 报红修：**阈值 30/60 → 90/180**（`scripts/check_csv_freshness.py`）+ 3 个 affordable 爬虫加 `publish_date` 列 + `read_existing/merge_rows` 防覆盖式写入丢历史（`crawl_gz_affordable_projects/targets.py` + `crawl_sz_affordable_projects.py`）+ workflow EXEMPT 白名单 +2（`gd_construction.csv` / `gz_housing_plan.csv`，用户拍板扩，已在 changelog 诚实记录「扩 EXEMPT 后邮件不再报警这两源」）。重跑 3 爬虫验证数据零丢失（5/8/5 行全替换仅 publish_date 列变）。本地 `check_csv_freshness` exit **2 → 0**。0 frontend 改动（versionCode 330） |
| v1.122.53 | 2026-09-14 | e2e visual-diff per-pixel 阈值 **12 → 60**（`tests/e2e/visual-diff.mjs` line 126：`dr+dg+db > 12` → `> 60`）。v1.122.52 修了整体 5% 阈值但 #73 (9-14) 仍 fail，根因是双层阈值未扫全：抗锯齿差异每个像素 RGB 求和 15-60 落到原 12 阈值外，整画面累计超 5%。新阈值 60 = 抗锯齿 noise < 60 视为无视觉变化，颜色块/字形差异 > 100 仍能拦住。`--threshold 0.05` 不变。代价：抗锯齿之外的小颜色变化（< 60 求和）也被吞——changelog 已诚实记录。0 frontend 改动（versionCode 332） |
| v1.122.52 | 2026-09-13 | e2e visual-diff 阈值 **2% → 5%**（`tests/e2e/visual-diff.mjs` 默认 + `.github/workflows/realty-app-e2e.yml` CI 显式）。**根因**：CI runner 字体渲染/抗锯齿与本机 Windows 不同导致 PNG 像素差异 > 2%，本地 diff=0.668% PASS、CI FAIL；#70 (9-11) / #71 (PR) / #72 (OTA 后) 连续 3 次 e2e fail 都不是真视觉回归。**代价**：真回归要超过 5% 才报警——changelog 已诚实记录，建议重大 UI 改动后立即本地跑 `--update-baseline` 提交新 baseline。0 frontend 实质改动（versionCode 331） |
...
| v1.122.39 | 2026-09-10 | 给 15 个 crawl_nbs_*.py 加 GBK stdout+stderr+stdin 三件套 UTF-8 包装（v1.122.37-39 只包了 stdout，漏 stderr）。NBS 系列 Windows 本地跑不再 GBK fail；同时 print(file=sys.stderr) 调用也能正确输出中文。**数据 csv 不动**——cron 月度发布正常节奏下 HEAD nbs_industrial/trade/pmi/cpi 已是 2026-07/08 最新数据。脚本只是恢复 cron 跑通能力，不 commit 数据（HEAD 已最新）。0 frontend 改动（versionCode 318） |
| v1.122.39 | 2026-09-10 | 修 pbc_fin_stats / pbc_region_sf GBK bug（同 v1.122.37/v1.122.38 sys.stdout UTF-8 包装）+ 补 sz_land_deals +1 行（2026-08-11 福田安托山四路深土[2026]-053 113100 万 起拍价成交）。PBOC 系列爬虫 GBK 包装至此全齐（nbs 15 个 / pbc 2 个 / safe 8 个）。amap 系列（15 个）未审计但仅按需触发平时 cron 不跑。0 frontend 改动（versionCode 318） |
| v1.122.38 | 2026-09-10 | 给 8 个 crawl_safe_*.py 批量加 sys.stdout UTF-8 包装（修 omo_rr/mlf_history 同款 GBK bug），同时补 6 个 SAFE CSV 月度数据：safe_ora +2 / safe_fx_market +2 / safe_bop_trade +2 / safe_bop +2（含 2023-06 回填）/ safe_iip +1 / safe_settle +1，最新到 2026-08。同时修 v1.122.37 时误判的 safe_ora source_url「可疑」回滚——SAFE 页面 URL 反映创建时间而非数据时间（页面 PubDate 2026-09-07 才是真发布日）。0 frontend 改动（versionCode 317） |
| v1.122.37 | 2026-09-10 | 修 3 个 PBOC 爬虫 bug + 补 6 个利率/PBOC 数据：crawl_shibor.py:112 IndentationError 修好（脚本原来根本不能跑 = shibor 91 天陈旧根因）；crawl_omo_rr.py + crawl_mlf_history.py 加 sys.stdout UTF-8 包装修 Windows GBK stdout bug（PBOC 公告标题含 \ufffd 让 cron job 红）。手动跑等同 cron 周末产出补 chinabond_yield +15、omo_rr +11、repo_fixing +34、shibor +34、pbc_fin_stats +1、pbc_region_sf +5 行（最新到 2026-09-10）。safe_* 6 个 csv 改动回滚（safe_ora source_url 是 2026-02 URL 装 2026-07/08 数据可疑）。mlf_history 562 天陈旧是 PBOC 政策变化（MLF 暂停）非脚本 bug。type-check 0 errors / vitest 145/1205 全过（versionCode 316） |
| v1.122.36 | 2026-09-10 | 补 LPR 2026-08 数据 + 修 `crawl_lpr_history.py` 格式化 bug：lpr_history.csv +1 行（85→86）；`f"{lpr1:g}"` 改成 `f"{lpr1:.1f}"`（修 3.0 渲染成 "3" 跟历史 baseline "3.0" 不一致导致字符串比较 fail）；同时 `tests/buildIntegrity.test.ts` LPR 硬断言从 2026-07 更新到 2026-08。9月数据等 PBOC 9-20 公布 + cron `0 2 16-20 * *` 自然触发。本机手动跑一次是 cron 提前产出，cron 链路本身已完整。type-check 0 errors / vitest 145/1205 全过（versionCode 315） |
| v1.122.35 | 2026-09-10 | 接链家在售二手 weekly cron（v0.10.0 脚本一直存在但**没有 workflow**）：新增 `.github/workflows/crawl-lianjia-weekly.yml`，每周二 03:00 UTC（北京时间 11:00）跑深/穗/珠 3 城 ×10 页（每页 ~30 条，共 ~900 条/周），`--append` 模式按 source_listing_id 去重保留旧行（不动 1240 DERIVED + 60 REAL）。流水线：抓取 → 校验 REAL≥50 → 跑 `compute_school_premium.py` 重算学区溢价 → 跑 `rebuild_listing_derivatives.py` 重算派生指标 → 自动 commit + push（数据无变化则 skip）。质量门禁 4 步必须 success。0 行改 `crawl_lianjia_listings.py`（已有 --append）。与既有 `crawl-weekly.yml` 周日 21:00 UTC 安居客错开 30h 互不冲突。type-check 0 errors / vitest 145/1205 全过（versionCode 314） |
| v1.122.34 | 2026-09-10 | 给宝安区 listings 补 ~40 套（findhouse 选深圳宝安区原本只有 2 套 REAL）：communities.csv 新增 10 个宝安小区 id 101-110（西乡/固戍/后瑞/塘尾/福永/新安/石岩/沙井/松岗/公明，city_id=2 深圳）；listings.csv 抽 20 套福田 + 20 套南山 DERIVED（共 40 套）重挂靠到宝安新社区，只改 community_id + title 前缀，**REAL listing 未动**（宝安原 2 套 REAL 在 id=27 联投东方华府一期保留）。跑 `compute_school_premium.py` 自动同步 `school_premium_community.csv`（+10 行）和 `school_premium_district.csv`（宝安区 listing_count 2→42, premium_ratio -35%→+25%；顺带修了斗门/香洲等 v0.11.0 后没重跑导致的陈旧 listing_count）。type-check 0 errors / vitest 145/1205 全过（versionCode 313） |
| v1.122.25 | 2026-09-10 | 6 个 GD 爬虫批量升级到 `_gbk_probe.fetch_text_gbk`：v1.122.23 只改了 `crawl_gd_construction.py` 的 fetch_text，但 `crawl_gd_economy / retail / industrial / real_estate_brief / fa_investment / services / provident_annual` 6 个爬虫仍用旧版 `for enc in ("utf-8", "gbk")`——会"看似成功"地解码 GBK body 为乱码。**v1.122.24 GD economy None key 修后**首次 cron weekly 会让所有 GD 爬虫进入"看起来 OK 但实际抓乱码"状态污染 CSV。修法：把 `scripts/_gbk_probe.py` 入仓（之前 untracked），6 个爬虫顶部加 `sys.path.insert + from _gbk_probe import fetch_text_gbk`，fetch_text body 全部替换为 1 行 `return fetch_text_gbk(url, ua=UA, ctx=CTX)`。本机验证所有 6 个爬虫都能跑通。**顺带发现**：之前 cron weekly 一直在用乱码 GD 数据，没人发现。145 files / 1205 tests 全过（versionCode 304） |
| v1.122.33 | 2026-09-10 | 修 4 个 NBS 月度测试硬断言（v1.122.29-32 续作）：nbsPmi / nbsTrade / nbsIndustrial / nbsIndustrialProfit —— find(2026-06 或 2026-05) + 数值 toBe → 范围断言（央行复核历史月数据可能微调）。修法同 v1.122.29-32 pattern。同期盘点其他文件：nbsCpi / nbsEnergy / nbsRetail / nbsGdp / motTransport 之前已修；nbsUnemployment / nbsAvgWage 是历史年值 / 年度源 append 不变；gdRetail / gdServices / szAffordable / gzAffordable 全用 if null 守卫；stats70.test.ts 用 hasStats70() 守卫；dailyWangqian / gzLandDeals 等都是 mock CSV → 不动。145 test files / 1205 tests 全过（versionCode 312） |
| v1.122.32 | 2026-09-10 | 修 8 个测试硬断言批量修复（v1.122.31 续作 batch）：chinabond / shibor / repoFixing (find 2026-07-24 + yield toBe) + safeBop / safeBopTrade / safeIip / safeOra (find 季度月 + 国际收支数值 toBe) + gdRealEstateBrief (trend(3).period 硬编码顺序)。修法同 v1.122.29 pattern：find 历史日 + yield toBe → 数值范围；trend 顺序 → contains 断言。同期盘点其他文件：gdServices / gdRetail / szAffordable / gzAffordable / nbsAvgWage 全部用历史日期 find + append 模式 + 央行不重公布历史数据 → 不动；safeForex 历史值 / safeUsdMid / pbcFinStats / safeSettle / safeFxMarket / mlfHistory 全用范围断言 → 已安全；omoRrHistory tenorDays=7 是业务假设（央行只发 7 天期）→ 不动。145 test files / 1205 tests 全过（versionCode 311） |
| v1.122.31 | 2026-09-10 | 修 2 个 mot / nbsGdp 测试硬断言（v1.122.30 续作）：motTransport.period=2026-H1 + 10 个数值 toBe + 标签验证 → regex + 数值范围 + 标签 regex；nbsGdp.period=2026-H1 + 18 个数值 toBe → regex + 范围 + 条件跳过 null。同期盘点还有 ~12 个真读 csv 测试硬编码具体数值（gdRetail / gdServices / gdRealEstateBrief / szAffordableProjects / gzAffordableTargets / nbsAvgWage / chinaBondYield / repoFixing / shibor / omoRrHistory / safeForex / safeUsdMid），留待后续 batch。145 test files / 1205 tests 全过（versionCode 310） |
| v1.122.30 | 2026-09-10 | 修 3 个测试 month/year 硬断言（v1.122.29 续作）：nbsEnergy / nbsRetail（month=2026-06 + 数值 toBe → month regex + 数值范围）/ zhBdcRegistration（year=2026 quarter=2 + unitsDelta 硬编码 → regex + 容差）。修法同 v1.122.29 pattern。同期扫尾 review 发现其他 month 硬编码都是 mock 数据（setSnapshot / 字符串 mock CSV / 假数据），不真读 csv，不受 cron 影响；educationOverview 年度源已 EXEMPT 候选等拍板。145 test files / 1205 tests 全过（versionCode 309） |
| v1.122.29 | 2026-09-10 | 修 10 个测试硬断言 year/period（防御性维护，cron monthly 自动 commit 后不会 break CI）：nbsCpi / nbsIncome / gdConstruction / gdEconomy / gdProvidentAnnual / gzHousingPlan / gzProvidentAnnual / szProvidentAnnual / zhProvidentDynamics / adversarialColorCitySupply。修法（与 v1.122.19 同 pattern）：period toBe → toMatch(/^20\\d{2}_H[12]\$/)；year toBe(2024) → toBeGreaterThanOrEqual(2015)；quarter / monthEnd → 范围断言；数值 toBe → toBeGreaterThan + toBeLessThan 范围；派生比例 / 差分 / rows 顺序假设全部改 regex 或容差。145 test files / 1205 tests 全过（versionCode 308） |
| v1.122.28 | 2026-09-10 | stats_70.csv 补 2026/7 完整月份数据（+140 行：70 同比 + 70 环比），同时修复 append_recent_stats70.py dedup bug（v1.122.8 起的真 bug：内层 dict key 用 date_str 导致 hugohe3 70 城市循环里"同比"被"环比"覆盖，dry-run 永远报"同比 0/环比 70 不完整"→ 完整性闸门拒绝追加）。修法：内层 key 改成 (date, fixed_base) 复合。本机绕开 github raw 抽风走 SSH clone hugohe3/70cityprice + --src 本地宽表。STALE 101d → 71d。type-check 0 errors / vitest 145/1205 全过（versionCode 307） |
| v1.122.27 | 2026-09-10 | 抽 crawl_common 共用 helper（v1.122.25 + v1.122.26 共改的 11 个 crawl_*.py 顶部 4 行样板收成 1 行 from crawl_common import fetch_text）：fetch_text 直接 re-export _gbk_probe.fetch_text_gbk（同一函数对象），行为完全兼容。crawl_pbc_region_sf.py 因为还保留 fetch_bytes 用 UA/CTX，单独处理 import as _common_fetch_text + 1 行 return 委托。crawl_gd_construction.py / crawl_gd_economy.py 有特殊逻辑（inline GBK probe / https 双栈）不参与本轮重构。11 modified + 1 added，净减 ~90 行样板。type-check 0 errors / vitest 145/1205 全过（versionCode 306） |
| v1.122.26 | 2026-09-10 | 5 个 PBC 爬虫 fetch_text 加 GBK 探测 fallback（crawl_lpr_history / crawl_mlf_history / crawl_omo_rr / crawl_pbc_fin_stats / crawl_pbc_region_sf）：和 v1.122.25 GD 同病——PBC 列表页 `http://www.pbc.gov.cn/zhengcehuobisi/...` 和 `diaochatongjisi/...` 同样 "Content-Type text/html 不标 charset 但 body 实际 GBK"，旧版 utf-8 优先看似成功但中文段乱码 → 后续"贷款市场报价利率 / 中期借贷便利 / 公开市场业务交易公告 / 金融统计数据报告 / 地区社会融资规模"正则全失效 → notices=0。复用 `_gbk_probe.fetch_text_gbk` 共用 helper。**本版本只 commit 代码修复，不 commit CSV 增量**：本机手跑 5 个 PBC 爬虫解锁 +10 行新数据（LPR +1 / OMO +3 / PBC fin_stats +1 / PBC region_sf +5，mlf +0），等用户拍板是否再开 v1.122.27 一次性 commit CSV 增量。145 test files / 1205 tests 全过（versionCode 305） |
| v1.122.20 | 2026-09-10 | `scripts/check.ps1` 默认 smoke URL 从 `http://127.0.0.1:9001/` 改为 `http://127.0.0.1:5174/`。v0.3.0 时代 `dev:h5 --port 9001` 显式指定过该端口；uni-app 后续默认 5174，没同步 check.ps1 默认值，导致 `check.ps1` 跑 smoke 必 `crash`（exit=2）。本机实测：修后 `check.ps1` 三门禁一次过（type-check / vitest / playwright smoke — interactive=140 / elements=6186 / errors=0）。同时入仓 5 个 self-hosted runner 安装/卸载脚本（install-self-hosted-runner / install-runner-service-admin / reconfig-runner-as-service / run-runner-interactive）+ `merge_anjuke_covers.py`；`.gitignore` 加 `/截图/` 屏蔽个人调试截图目录（versionCode 299） |
...
| v1.122.8 | 2026-09-09 | 全站 CSV 数据陈旧度统一扫描：`scripts/check_csv_freshness.py` 扫 41 个 CSV 按 date/publish_date/effective_date/year/period/month/quarter 字段算 OK/WARN/STALE 三档（默认 30/60 天），退出码与 CI 邮件告警挂钩；`.github/workflows/check-csv-freshness.yml` 每天 02:30 UTC 跑，artifact 归档 30 天。本机首次扫描结果：✅ OK 2 / ⚠️ WARN 6 / ❗ STALE 35（含 stats_70 100 天、gz_provident_annual 531 天、provident_fund_rates 489 天等）。F-CSV-FRESHNESS 写进 FEATURE_CATALOG（versionCode 287） |
| v1.122.9 | 2026-09-09 | check_csv_freshness.py 字段策略收紧：把 year / period / month / quarter 这类「统计期标识」从默认候选剔除（这些字段值是「项目立项年」而非「数据爬取时间」，会误报 STALE），改成优先 publish_date / as_of_date / effective_date / date；UNKNOWN 不再计入 worst 退出码，避免「立项 1 年的项目 CSV」把 cron 推红。同次把 scripts/check_stats70_freshness.py 的 zoneinfo('Asia/Shanghai') 改成 date.today()（修 Windows Python 缺 tzdata 崩的老问题）+ 新增 scripts/append_recent_stats70.py 增量追加工具带 70×2 完整性闸门（hugohe3 2026/7 同比未补时不追加，避免引入半截数据）（versionCode 288） |
| v1.122.10 | 2026-09-09 | 把 append_recent_stats70.py 接进 crawl-monthly-stats70.yml cron：download 因历史覆盖不全失败时由 append 增量补齐兜底；`set -o pipefail` 移除并加 `\|\| true` 让 download 失败不阻断 append；本月底 publish_day 后 70 城指数自动 append 不再依赖 download 整表 replace（versionCode 289） |
| v1.122.11 | 2026-09-09 | 本机手动补齐广州/广州教育/广州公积金/广州保障房/广州新房库存/广州土地出让等月度陈旧数据（gz_land_deals 4 条新增 / gz_provident_annual 6 行历史补全到 2018-2024 / gz_new_house_inventory +11 行 / gz_housing_plan / education_overview 等）。同时修 nbs_income / nbs_fa_investment / nbs_real_estate 的 INDEX_URL 从 /sj/ 改到 /sj/zxfb/ 子栏目（+unemployment/fa/real_estate 月度各 +1 行 2026-07~08）（versionCode 290） |
| v1.122.12 | 2026-09-09 | 装 xlrd==2.0.1 在 .venv（默认 pip 缺 tzdata + null byte bug 仍存在，绕开系统 pip 直接装 venv 里的）让 gz_affordable_targets / gz_affordable_projects 两个脚本能跑；targets -1 行清理无 actual 的 2025 raised 计划项（versionCode 291） |
...
| v1.121.146 | 2026-07-29 | 广州库存接入商业/办公/车位分项（同源住建局可售接口）；供需页 KPI；快照刷新至 2026-07-27（versionCode 267） |
| v1.121.145 | 2026-07-29 | 总览首屏精简（美团式）：去掉指南/三按钮/进阶占位/工作台；次要入口迁设置「首页与分析」；频道仍跳独立页（versionCode 266） |
| v1.121.144 | 2026-07-29 | 去掉概览「分组折叠 / 全部展开收起 / 滚锚点快捷钮」；进阶分析改跳独立页；频道跳页承接 143（versionCode 265） |
...
| v1.121.136 | 2026-07-27 | 第三批瘦身：dashboard.vue 真删 15 张宏卡 HTML + 对应 refs/imports（MLF/OMO/Shibor/国债/FR-FDR/央行金融统计/广东社融/8 张外汇）；15,392 → 14,186 行（-1,206）；15 张宏卡全部进 macro-rates 或 macro-fx 子页；预算硬规则收紧到 ≤ 4 宏卡 / ≤ 14.5k 行 |
| v1.121.135 | 2026-07-27 | 加新数据：macro-rates 子页从骨架升级为 6 张利率卡（LPR/MLF/逆回购/Shibor/中债国债/回购定盘 FR/FDR）；25 → 390 行；38 KPI |
| v1.121.134 | 2026-07-27 | 加新数据：macro-fx 子页从骨架升级为 8 张汇市卡（外储/官方储备/中间价/外汇市场/结售汇/BOP/IIP/货服贸易）；23 → 453 行 |
...
| v1.121.125 | 2026-07-26 | Shibor 同业拆放利率 |
| v1.121.124 | 2026-07-26 | 中债国债收益率（关键期限 + 利差） |
| v1.121.123 | 2026-07-26 | 采购经理指数（含建筑业） |
| v1.121.122 | 2026-07-26 | 海关货物进出口（经 NBS） |
| v1.121.121 | 2026-07-26 | 国际收支平衡表季报 |
| v1.121.120 | 2026-07-26 | 国际投资头寸季报 |
| v1.121.119 | 2026-07-26 | 国际收支货物和服务贸易月度 |
| v1.121.118 | 2026-07-26 | 官方储备资产分项（黄金/SDR/合计） |
| v1.121.117 | 2026-07-26 | 价格 Tab/金刚区修复 + Tab 验收用例 |
| v1.121.116 | 2026-07-26 | 汇率中间价扩欧元/港元/日元 |
| v1.121.115 | 2026-07-26 | 搜索框/详情底栏文字垂直居中 + 门禁 |
| v1.121.114 | 2026-07-26 | 美元中间价日度；外储补 2022-11 |
| v1.121.113 | 2026-07-26 | 外汇市场成交概况；外储扩至 2022 |
| v1.121.112 | 2026-07-26 | 结售汇补涉外收付款；外储数字空格解析 |
| v1.121.111 | 2026-07-26 | 银行结售汇月度；外储历史扩样 |
| v1.121.110 | 2026-07-26 | 外管局外汇储备规模；库存入口 smoke 修复 |
| v1.121.109 | 2026-07-26 | 地区社融扩至苏浙京沪对照 |
| v1.121.108 | 2026-07-26 | 广东社融占全国对照；金融统计补外储/汇率 |
| v1.121.107 | 2026-07-26 | 广东地区社融增量；金融统计旧期补齐 |
| v1.121.106 | 2026-07-26 | 央行金融统计（社融/M2/住户贷款）接入 |
| v1.121.105 | 2026-07-26 | 央行 MLF + 公开市场 7 天逆回购；库存入口 E2E 脚本 |
| v1.121.104 | 2026-07-26 | 珠海价格备案全量 336 条回填 |
| v1.121.103 | 2026-07-26 | 珠海备案扩样+分区；挂牌 cover_url 列表/详情接线 |
| v1.121.102 | 2026-07-26 | 珠海商品房价格备案 HTML 接入；库存/网签/LPR 刷新 |
| v1.121.101 | 2026-07-26 | 列表户型筛对齐地图；珠海登记历史季五区 |
| v1.121.100 | 2026-07-26 | 对照贝壳整体验收：列表详情地图/珠海网签 |
| v1.121.99 | 2026-07-26 | 珠海不动产登记季报（PNG 合计）作网签量能备选 |
| v1.121.98 | 2026-07-26 | 学校按城过滤 + 详情标签 pill + backlog 清扫 |
| v1.121.97 | 2026-07-26 | 地图找房主路径：筛总价户型+小区底栏+进详情（对照贝壳） |
| v1.121.96 | 2026-07-26 | 地图挂牌均价可读性 + MAP_ACCEPTANCE 用例补齐 |
| v1.121.95 | 2026-07-26 | 浅色模式一眼可辨：JS 注入 token + 去掉首页深色硬编码 |
| v1.121.94 | 2026-07-26 | 房源二手/新房分类 + 筛选修复 + 测试验收/功能文档 |
| v1.121.93 | 2026-07-26 | NBS 社消装潢材料/家具零售 |
| v1.121.92 | 2026-07-26 | NBS 住宅施工/新开工/竣工分项 |
| v1.121.91 | 2026-07-26 | NBS 到位资金拆分（贷款/定金/按揭/自筹） |
| v1.121.90 | 2026-07-26 | NBS 住宅分项与个人按揭贷款 |
| v1.121.89 | 2026-07-26 | NBS 工业生产者价格（PPI，含建材分项） |
| v1.121.88 | 2026-07-26 | NBS 居民消费价格（含居住/房租）与收入回填 |
| v1.121.87 | 2026-07-26 | 国家统计局全国居民收入和消费（含居住） |
| v1.121.86 | 2026-07-26 | NBS 房地产补施工/新开工/竣工面积 |
| v1.121.85 | 2026-07-26 | 国家统计局全国固定资产投资卡 |
| v1.121.84 | 2026-07-26 | 房源关键字筛选；首页「房源」搜索贯通 |
| v1.121.83 | 2026-07-26 | 总览多入口：搜索+频道+金刚区（F-ENTRY-01） |
| v1.121.82 | 2026-07-26 | 官方宏观卡分组展示与脚注精简 |
| v1.121.81 | 2026-07-26 | 经济运行补常住人口/城镇化；回填2025前三季度 |
| v1.121.80 | 2026-07-26 | 广东经济运行补人均可支配收入（城镇/农村） |
| v1.121.79 | 2026-07-26 | 广东经济运行简况（GDP/房开投资/CPI） |
| v1.121.78 | 2026-07-26 | 广东建筑业生产运行简况（产值/房屋建筑业/珠三角） |
| v1.121.77 | 2026-07-26 | 广东固投简况；房地产统计局1-4/1-5累月补齐 |
| v1.121.76 | 2026-07-26 | 广东运行简况多期回填+折叠；统计局上半年最新期 |
| v1.121.75 | 2026-07-26 | 省年报折叠；深圳公积金2024种子+同比；广东房地产运行简况 |
| v1.121.74 | 2026-07-26 | 珠海同月末对照；广东全省公积金年报 |
| v1.121.73 | 2026-07-26 | 珠海公积金动态；广州公租房计提补全 |
...
完成 22 个移动端路由与五个一级入口的全量改版，新增行情聚合页和仅本机收藏/浏览历史，并统一翡翠绿 + 科技蓝的浅色、深色与响应式设计系统；同时修复 daily 网签 merge、70 城发布窗口和 weekly P0/P1/P2 告警策略。详见 [changelog/2026-08-18-v1.122.0-移动端全量改版与CI告警治理.md](./changelog/2026-08-18-v1.122.0-移动端全量改版与CI告警治理.md)。

### v1.121.157 (2026-07-29)

交通运输部「交通运输经济运行情况」接入 `macro-industry`（货运/港口/集装箱/人员流动/交通固投）。详见 [changelog/2026-07-29-v1.121.157-mot-transport.md](./changelog/2026-07-29-v1.121.157-mot-transport.md)。
...
国家统计局「城镇单位就业人员年平均工资」接入 `macro-industry`（非私营/私营 + 房地产/建筑业行业）；兼容实际增长与可比口径。详见 [changelog/2026-07-29-v1.121.153-nbs-avg-wage.md](./changelog/2026-07-29-v1.121.153-nbs-avg-wage.md)。

### v1.121.152 (2026-07-29)

国家统计局「能源生产情况」月报接入 `macro-industry`（原煤/原油/天然气/发电量）；单月稿跳过累计句、兼容持平。详见 [changelog/2026-07-29-v1.121.152-nbs-energy.md](./changelog/2026-07-29-v1.121.152-nbs-energy.md)。
...
广东统计局「规模以上服务业运行简况」接入 `macro-region`（营收同比 + 租赁商务/房地产服务分项，≠房价）；周更 CI。详见 [changelog/2026-07-29-v1.121.149-gd-services.md](./changelog/2026-07-29-v1.121.149-gd-services.md)。

### v1.121.148 (2026-07-29)

消费品专栏补「实现」句式与 2025Q3；家具/装潢限上零售分项；供需页可展开业态分区。详见 [changelog/2026-07-29-v1.121.148-retail-q3-furniture-supply-nonres.md](./changelog/2026-07-29-v1.121.148-retail-q3-furniture-supply-nonres.md)。

### v1.121.147 (2026-07-29)

广东统计局「规上工业生产」「消费品市场」专栏接入 `macro-region`（分项同比 + 多期）；周更 CI。详见 [changelog/2026-07-29-v1.121.147-gd-industrial-retail.md](./changelog/2026-07-29-v1.121.147-gd-industrial-retail.md)。

### v1.121.146 (2026-07-29)

广州新房库存 CSV/模块接入商业、办公、车位分项（同源可售/未售/签约接口）；供需页展示非住宅 KPI；历史日缺列不报虚假业态环比。详见 [changelog/2026-07-29-v1.121.146-gz-inventory-nonres.md](./changelog/2026-07-29-v1.121.146-gz-inventory-nonres.md)。

### v1.121.145 (2026-07-29)

总览首屏精简：去掉指南 banner、三按钮、进阶占位与概览工作台；次要入口迁设置「首页与分析」。详见 [changelog/2026-07-29-v1.121.145-home-shell-slim.md](./changelog/2026-07-29-v1.121.145-home-shell-slim.md)。

### v1.121.144 (2026-07-29)

去掉概览「快捷滚锚点 + 全部展开/收起」折叠套路；进阶分析改为进深度可视化/数据工具独立页。详见 [changelog/2026-07-29-v1.121.144-remove-overview-fold.md](./changelog/2026-07-29-v1.121.144-remove-overview-fold.md)。
...
国家统计局规上工业企业利润累计通稿接入：利润/营收/利润率与三大门类分项；仪表盘新卡接在工业增加值后，明示累计口径与 ≠ 房价三轴。详见 [changelog/2026-07-26-v1.121.128-规上工业企业利润.md](./changelog/2026-07-26-v1.121.128-规上工业企业利润.md)。

### v1.121.127 (2026-07-26)

国家统计局规模以上工业增加值通稿接入：当月同比/环比/累计同比与三大门类分项；仪表盘新卡接在 PMI 后，明示 ≠ 房价三轴。详见 [changelog/2026-07-26-v1.121.127-规上工业增加值.md](./changelog/2026-07-26-v1.121.127-规上工业增加值.md)。

### v1.121.126 (2026-07-26)

中国货币网回购定盘利率接入：FR001/007/014 与银银间 FDR001/007/014；仪表盘新卡，明示 FDR 基于 DR 加权成交、≠ 房价三轴。详见 [changelog/2026-07-26-v1.121.126-回购定盘利率FR-FDR.md](./changelog/2026-07-26-v1.121.126-回购定盘利率FR-FDR.md)。

### v1.121.125 (2026-07-26)

中国货币网 Shibor 接入：最新 JSON + 历史 His API；仪表盘新卡展示隔夜～1 年关键期限，明示 ≠ 房价三轴。详见 [changelog/2026-07-26-v1.121.125-Shibor同业拆放利率.md](./changelog/2026-07-26-v1.121.125-Shibor同业拆放利率.md)。

### v1.121.124 (2026-07-26)

中债国债收益率曲线接入：监管展示表 HTML 覆盖最新日，API 回填历史；仪表盘新卡展示关键期限与 10Y−1Y 利差，明示 ≠ 房价三轴。详见 [changelog/2026-07-26-v1.121.124-中债国债收益率.md](./changelog/2026-07-26-v1.121.124-中债国债收益率.md)。

### v1.121.123 (2026-07-26)

国家统计局「中国采购经理指数」月度通稿接入（制造业 / 非制造业 / 建筑业 / 综合产出）；仪表盘新卡，临界点 50，建筑业商务活动明示 ≠ 房价三轴。详见 [changelog/2026-07-26-v1.121.123-采购经理指数含建筑业.md](./changelog/2026-07-26-v1.121.123-采购经理指数含建筑业.md)。

### v1.121.122 (2026-07-26)

海关总署官网 WAF 不可直抓；改从国家统计局「国民经济运行」通稿接入海关口径货物进出口（当月/累计、进出口顺差）；仪表盘新卡，口径 ≠ 房价三轴，并与外管局货服贸易美元口径区分。详见 [changelog/2026-07-26-v1.121.122-海关货物进出口经NBS.md](./changelog/2026-07-26-v1.121.122-海关货物进出口经NBS.md)。

### v1.121.121 (2026-07-26)

外管局「国际收支平衡表」季度通稿接入（经常账户 / 货服 / 初次二次收入 / 资本和金融账户）；正式数优先于初步数；仪表盘新卡，口径 ≠ 房价三轴，并与月度货服贸易卡区分。详见 [changelog/2026-07-26-v1.121.121-国际收支平衡表季报.md](./changelog/2026-07-26-v1.121.121-国际收支平衡表季报.md)。

### v1.121.120 (2026-07-26)

外管局「国际投资头寸表」季末通稿接入（对外金融资产/负债/净资产及一级分项）；仪表盘新卡，口径 ≠ 房价三轴。详见 [changelog/2026-07-26-v1.121.120-国际投资头寸季报.md](./changelog/2026-07-26-v1.121.120-国际投资头寸季报.md)。

### v1.121.119 (2026-07-26)

外管局「国际收支货物和服务贸易」月度通稿接入（货物/服务进出口与顺差）；仪表盘新卡，口径 ≠ 房价三轴。详见 [changelog/2026-07-26-v1.121.119-国际收支货物服务贸易.md](./changelog/2026-07-26-v1.121.119-国际收支货物服务贸易.md)。

### v1.121.118 (2026-07-26)

外管局「官方储备资产」月表接入（外储/IMF/SDR/黄金市值与万盎司/合计）；仪表盘新卡，口径 ≠ 房价三轴。详见 [changelog/2026-07-26-v1.121.118-官方储备资产分项.md](./changelog/2026-07-26-v1.121.118-官方储备资产分项.md)。
...
### v1.121.115 (2026-07-26)

房源搜索框、首页搜索、详情「查看参考页面/复制链接」等单行控件文字垂直居中；全局 `button` 默认 flex 居中；新增 `uiVerticalAlign` 源码门禁。详见 [changelog/2026-07-26-v1.121.115-单行控件垂直居中.md](./changelog/2026-07-26-v1.121.115-单行控件垂直居中.md)。

### v1.121.114 (2026-07-26)

外管局人民币对美元中间价日度查询接入；外汇储备补回 2022-11 缺期。详见 [changelog/2026-07-26-v1.121.114-美元中间价与外储补缺.md](./changelog/2026-07-26-v1.121.114-美元中间价与外储补缺.md)。

### v1.121.113 (2026-07-26)

外管局「中国外汇市场交易概况」月度成交接入；外汇储备新闻分页扩至约 44 期（至 2022-10）。详见 [changelog/2026-07-26-v1.121.113-外汇市场成交与外储扩样.md](./changelog/2026-07-26-v1.121.113-外汇市场成交与外储扩样.md)。

### v1.121.112 (2026-07-26)

结售汇卡补「银行代客涉外收付款」；外汇储备通稿兼容数字间空格（补回 2024-03/04）。详见 [changelog/2026-07-26-v1.121.112-涉外收付款与外储解析.md](./changelog/2026-07-26-v1.121.112-涉外收付款与外储解析.md)。

### v1.121.111 (2026-07-26)

外管局「银行结售汇」月度通稿接入（结汇/售汇/派生顺差）；外汇储备新闻分页扩至约 29 期。详见 [changelog/2026-07-26-v1.121.111-银行结售汇与外储扩样.md](./changelog/2026-07-26-v1.121.111-银行结售汇与外储扩样.md)。

### v1.121.110 (2026-07-26)

国家外汇管理局「外汇储备规模」月度通稿接入仪表盘（可与官方储备资产表交叉）；库存金刚区 smoke 选择器与视口断言修复。详见 [changelog/2026-07-26-v1.121.110-外管局外储与库存smoke.md](./changelog/2026-07-26-v1.121.110-外管局外储与库存smoke.md)。

### v1.121.109 (2026-07-26)

地区社融 XLSX 扩抓苏浙京沪，仪表盘广东卡展示同期省份对照。详见 [changelog/2026-07-26-v1.121.109-地区社融苏浙京沪对照.md](./changelog/2026-07-26-v1.121.109-地区社融苏浙京沪对照.md)。

### v1.121.108 (2026-07-26)

广东社融增量相对全国同期占比（派生）；金融统计报告补外汇储备与美元兑人民币。详见 [changelog/2026-07-26-v1.121.108-广东社融占全国与外储.md](./changelog/2026-07-26-v1.121.108-广东社融占全国与外储.md)。

### v1.121.107 (2026-07-26)

广东地区社融增量（XLSX）接入；全国金融统计兼容无社融段旧月报并补齐 2025-07/08。详见 [changelog/2026-07-26-v1.121.107-广东社融增量与金融统计补齐.md](./changelog/2026-07-26-v1.121.107-广东社融增量与金融统计补齐.md)。

### v1.121.106 (2026-07-26)

央行「金融统计数据报告」接入仪表盘（社融/M2/住户中长期贷款结构；明示 ≠ 房价）。详见 [changelog/2026-07-26-v1.121.106-央行金融统计社融M2.md](./changelog/2026-07-26-v1.121.106-央行金融统计社融M2.md)。

### v1.121.105 (2026-07-26)

央行 MLF 开展情况 + 公开市场 7 天期逆回购接入仪表盘（与 LPR 对照）；库存金刚区补 smoke 脚本。详见 [changelog/2026-07-26-v1.121.105-央行MLF与库存入口门禁.md](./changelog/2026-07-26-v1.121.105-央行MLF与库存入口门禁.md)。

### v1.121.104 (2026-07-26)

珠海商品房价格备案专栏全部分页回填（336 条 / 13252 套）。详见 [changelog/2026-07-26-v1.121.104-珠海备案全量回填.md](./changelog/2026-07-26-v1.121.104-珠海备案全量回填.md)。

### v1.121.103 (2026-07-26)

珠海价格备案扩至 144 条并按地址分区展示；挂牌 `cover_url` 贯通列表/详情（真图仍受链家 CAPTCHA 限制）。详见 [changelog/2026-07-26-v1.121.103-备案扩样与挂牌封面接线.md](./changelog/2026-07-26-v1.121.103-备案扩样与挂牌封面接线.md)。

### v1.121.102 (2026-07-26)

珠海住建局商品房价格备案公示 HTML 表摘要接入仪表盘；广州库存/深广网签 merge/LPR 刷新。详见 [changelog/2026-07-26-v1.121.102-珠海价格备案与数据刷新.md](./changelog/2026-07-26-v1.121.102-珠海价格备案与数据刷新.md)。

### v1.121.101 (2026-07-26)

列表户型筛选与地图找房同档；珠海不动产登记五区明细回填 2025Q1–2026Q2。详见 [changelog/2026-07-26-v1.121.101-列表户型与珠海分区.md](./changelog/2026-07-26-v1.121.101-列表户型与珠海分区.md)。

### v1.121.100 (2026-07-26)

对照贝壳/链家过一遍：列表价区与标签、详情图集空态、地图找房文案、珠海网签入口不跳深圳；验收 MD 与门禁补齐。详见 [changelog/2026-07-26-v1.121.100-对照贝壳整体验收.md](./changelog/2026-07-26-v1.121.100-对照贝壳整体验收.md)。

### v1.121.99 (2026-07-26)

珠海不动产登记中心季度「新增商品房 / 存量房转移」合计接入仪表盘（PNG 人工抄录）；日更专网仍不可用。详见 [changelog/2026-07-26-v1.121.99-珠海不动产登记季报.md](./changelog/2026-07-26-v1.121.99-珠海不动产登记季报.md)。

### v1.121.98 (2026-07-26)

清扫活跃 backlog：学校重点校按城过滤、详情挂牌标签 pill、教育/库存门禁。详见 [changelog/2026-07-26-v1.121.98-学校按城与详情标签.md](./changelog/2026-07-26-v1.121.98-学校按城与详情标签.md)。

### v1.121.97 (2026-07-26)

地图找房对齐贝壳主路径：默认找房、总价/户型快筛、点小区出房源底栏、点房源进详情。详见 [changelog/2026-07-26-v1.121.97-地图找房主路径.md](./changelog/2026-07-26-v1.121.97-地图找房主路径.md)。

### v1.121.96 (2026-07-26)

地图「挂牌均价」黑糊修复：半透明热力、只画有价社区、图例禁 0k-0k；补 MAP_ACCEPTANCE（UI/功能/逻辑）。详见 [changelog/2026-07-26-v1.121.96-地图挂牌均价可读与验收.md](./changelog/2026-07-26-v1.121.96-地图挂牌均价可读与验收.md)。

### v1.121.95 (2026-07-26)

浅色模式「一眼可辨」：JS 注入 token、去掉首页深色硬编码渐变、强化 THEME_ACCEPTANCE。详见 [changelog/2026-07-26-v1.121.95-浅色模式一眼可辨修复.md](./changelog/2026-07-26-v1.121.95-浅色模式一眼可辨修复.md)。

### v1.121.94 (2026-07-26)

房源「二手房/新房」分类与筛选修复、验收文档总册。详见 [changelog/2026-07-26-v1.121.94-房源二手新房分类与筛选验收.md](./changelog/2026-07-26-v1.121.94-房源二手新房分类与筛选验收.md)。

### v1.121.93 (2026-07-26)

NBS 社消限额以上建筑装潢材料/家具零售接入（≠房价）。详见 [changelog/2026-07-26-v1.121.93-NBS社消装潢家具零售.md](./changelog/2026-07-26-v1.121.93-NBS社消装潢家具零售.md)。

### v1.121.92 (2026-07-26)

NBS 全国房地产卡补住宅施工/新开工/竣工分项与住宅施工占比。详见 [changelog/2026-07-26-v1.121.92-NBS住宅施工新开工竣工.md](./changelog/2026-07-26-v1.121.92-NBS住宅施工新开工竣工.md)。

### v1.121.91 (2026-07-26)

NBS 全国房地产卡补到位资金结构（国内贷款/定金预收款/按揭/自筹）；住宅行展示住宅投资。详见 [changelog/2026-07-26-v1.121.91-NBS到位资金拆分.md](./changelog/2026-07-26-v1.121.91-NBS到位资金拆分.md)。

### v1.121.90 (2026-07-26)

NBS 全国房地产卡补住宅销售/待售/投资与个人按揭贷款分项，并派生住宅合同均价。详见 [changelog/2026-07-26-v1.121.90-NBS住宅分项与个人按揭.md](./changelog/2026-07-26-v1.121.90-NBS住宅分项与个人按揭.md)。

### v1.121.89 (2026-07-26)

NBS 全国工业生产者价格（PPI，含购进与非金属矿物制品业建材分项；≠房价）接入。详见 [changelog/2026-07-26-v1.121.89-NBS工业生产者价格PPI.md](./changelog/2026-07-26-v1.121.89-NBS工业生产者价格PPI.md)。

### v1.121.88 (2026-07-26)

NBS 全国居民消费价格（含居住/房租分项；房租≠房价）接入；居民收入回填 2025 多期。详见 [changelog/2026-07-26-v1.121.88-NBS居民消费价格与收入回填.md](./changelog/2026-07-26-v1.121.88-NBS居民消费价格与收入回填.md)。

### v1.121.87 (2026-07-26)

全国居民收入和消费支出接入（含居住消费；≠房价）。详见 [changelog/2026-07-26-v1.121.87-全国居民收入消费.md](./changelog/2026-07-26-v1.121.87-全国居民收入消费.md)。

### v1.121.86 (2026-07-26)

全国房地产卡补房屋施工/新开工/竣工面积（官方表字段入库）。详见 [changelog/2026-07-26-v1.121.86-NBS施工新开工竣工.md](./changelog/2026-07-26-v1.121.86-NBS施工新开工竣工.md)。

### v1.121.85 (2026-07-26)

国家统计局全国固定资产投资基本情况接入仪表盘（与广东固投对照；≠房价）。详见 [changelog/2026-07-26-v1.121.85-全国固定资产投资.md](./changelog/2026-07-26-v1.121.85-全国固定资产投资.md)。

### v1.121.84 (2026-07-26)

房源 Tab 关键字筛选（标题/小区/行政区）；首页「房源」搜索写入 pending 并自动应用。详见 [changelog/2026-07-26-v1.121.84-房源关键字筛选.md](./changelog/2026-07-26-v1.121.84-房源关键字筛选.md)。

### v1.121.83 (2026-07-26)

总览首页多入口（定位/搜索/频道/金刚区）；工作台默认折叠。验收见 [docs/DASHBOARD_ENTRY_IA.md](./docs/DASHBOARD_ENTRY_IA.md)。变更说明 [changelog/2026-07-26-v1.121.83-总览多入口.md](./changelog/2026-07-26-v1.121.83-总览多入口.md)。

### v1.121.82 (2026-07-26)

仪表盘官方宏观卡分组（全国/广东）与脚注精简，城乡人口进折叠。详见 [changelog/2026-07-26-v1.121.82-宏观卡展示整理.md](./changelog/2026-07-26-v1.121.82-宏观卡展示整理.md)。

### v1.121.81 (2026-07-26)

经济运行卡补年报常住人口/城镇化率，回填 2025 前三季度 GDP 期次。详见 [changelog/2026-07-26-v1.121.81-常住人口与城镇化.md](./changelog/2026-07-26-v1.121.81-常住人口与城镇化.md)。

### v1.121.80 (2026-07-26)

广东经济运行卡补居民人均可支配收入（全省/城镇/农村 · 名义/实际同比）。详见 [changelog/2026-07-26-v1.121.80-广东人均可支配收入.md](./changelog/2026-07-26-v1.121.80-广东人均可支配收入.md)。

### v1.121.79 (2026-07-26)

接入省统计局经济运行简况（含 GDP 期次：GDP/三产、房开投资、规上工业、社消零、CPI）。详见 [changelog/2026-07-26-v1.121.79-广东经济运行简况.md](./changelog/2026-07-26-v1.121.79-广东经济运行简况.md)。

### v1.121.78 (2026-07-26)

接入省住建厅建筑业生产运行简况（总产值/房屋建筑业/土木/珠三角）。详见 [changelog/2026-07-26-v1.121.78-广东建筑业生产运行简况.md](./changelog/2026-07-26-v1.121.78-广东建筑业生产运行简况.md)。

### v1.121.77 (2026-07-26)

接入省统计局固定资产投资运行简况；房地产简况补齐 2026 年 1–4/1–5 并强化双列表爬虫。详见 [changelog/2026-07-26-v1.121.77-广东固投简况与RE累月补齐.md](./changelog/2026-07-26-v1.121.77-广东固投简况与RE累月补齐.md)。

### v1.121.76 (2026-07-26)

广东房地产运行简况回填多期并默认折叠；接入省统计局 2026 上半年为最新期。详见 [changelog/2026-07-26-v1.121.76-广东运行简况多期与折叠.md](./changelog/2026-07-26-v1.121.76-广东运行简况多期与折叠.md)。

### v1.121.75 (2026-07-26)

广东全省公积金年报默认折叠；回填深圳 2024 年报种子并展示上年对照；接入省住建厅房地产市场运行简况。详见 [changelog/2026-07-26-v1.121.75-省年报折叠深圳2024与广东运行简况.md](./changelog/2026-07-26-v1.121.75-省年报折叠深圳2024与广东运行简况.md)。

### v1.121.74 (2026-07-26)

珠海公积金动态增加同月末对照；接入广东省住房公积金年报全省指标。详见 [changelog/2026-07-26-v1.121.74-珠海同月末对照与广东全省年报.md](./changelog/2026-07-26-v1.121.74-珠海同月末对照与广东全省年报.md)。

### v1.121.73 (2026-07-26)

接入珠海住房公积金动态（缴存/贷款/个贷率，含 2025 全年）；广州年报公租房计提补全为 42.46 亿。详见 [changelog/2026-07-26-v1.121.73-珠海公积金动态与广州公租房计提.md](./changelog/2026-07-26-v1.121.73-珠海公积金动态与广州公租房计提.md)。
