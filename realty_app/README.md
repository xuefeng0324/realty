...
| 版本 | 发布日期 | 说明 |
|------|----------|------|
| v1.122.24 | 2026-09-10 | `crawl_gd_economy.py` `atomic_write` 加 None key 防御过滤：探测发现 `gd_economy.csv` 第 5 行（2026_01_07）因**多一个逗号**导致 DictReader 多读一列（header 32 fields, 该行 33 fields），空 header 被 DictReader 当作 `None` key → DictWriter 写时 `key=None` 不在 fieldnames 抛 `ValueError: dict contains fields not in fieldnames: None`。**v1.122.23 GBK fix 让本就有病的 CSV 显式触发 bug**——之前乱码时代码根本解析不出 GDP（return None），atomic_write 拿到空 row 不爆；GBK fix 后 parse_brief 找到 GDP 进 merged，atomic_write 写入第 5 行（带 None key）爆。**首次 cron weekly 就会爆**。修法：atomic_write 加防御性过滤 None key / 未知 key / 补默认空值；load_existing 也加同样防护；atomic_write 重写 CSV 时自动清掉 None key 列。修后爬虫正常输出 5 行（包含 v1.122.23 GBK fix 解锁的 2026 H1 GDP 72281.05 亿 yoy 4.5% 等）。145 files / 1205 tests 全过（versionCode 303） |
