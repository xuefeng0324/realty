...
| 版本 | 发布日期 | 说明 |
|------|----------|------|
| v1.122.23 | 2026-09-10 | `crawl_gd_construction.py` `fetch_text` 加 GBK 探测 fallback：源站 `zfcxjst.gd.gov.cn` HTTP header 标 utf-8 但 body 实际 GBK——utf-8 解码 GBK 字节"看似成功"（GBK 单字节 ASCII 段兼容 utf-8）但中文段全乱码，导致后续正则 `建筑业生产运行简况` 永远不 match。本轮探测发现首页"建筑业"出现14 次但匹配失败（乱码形态）。修法：先按 gbk/gb18030 + `errors='replace'` 容错解码，探测中文高频词（建筑业/经济/人民政府/广东）任一出现即选用；都不命中 fallback utf-8 replace。本轮检查源站首页 1-5 页只列 4 条建筑业 brief（2026_Q1/2025/2025_Q1/2024），**没有 H1**——134 天 STALE 是源站延期发布，不是爬虫 bug。cron weekly 自动捕获等 H1（versionCode 302） |
