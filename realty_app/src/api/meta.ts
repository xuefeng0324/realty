...
export async function getPeriods(params: {
  ...
  cityId?: number;
  limit?: number;
}): Promise<{ type: string; items: string[] }> {
  return apiGet("/api/v1/periods", { ...params, limit: params.limit ?? 20 });
}
...
export async function getSources(params: { cityId: number }): Promise<{
  cityId: number;
  items: SourceStatItem[];
}> {
  return apiGet("/api/v1/sources", params);
}

export async function getCoverage(params: { cityId: number; source?: string }): Promise<CoverageResponse> {
  return apiGet("/api/v1/coverage", params);
}