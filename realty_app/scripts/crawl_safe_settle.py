...
用法：
  ...
  python scripts/crawl_safe_settle.py --max 30 --news-pages 16
"""
...
OUT = ROOT / "static" / "seed" / "safe_settle.csv"
...
def parse_body(html: str, source_url: str) -> dict[str, str] | None:
    ...
    if not m:
        m = re.search(
            r"(20\d{2})\s*年\s*(\d{1,2})\s*月[，,]\s*银行结汇\s*([\d.]+)\s*亿美元[，,]\s*售汇\s*([\d.]+)\s*亿美元",
            text,
        )
    if not m:
        return None
...
def main() -> int:
    ...
    ap.add_argument("--news-pages", type=int, default=16)
