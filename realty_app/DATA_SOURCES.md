...
| `static/gd_economy.csv` | `gdEconomy.ts` + 仪表盘「广东经济运行」卡（**多期默认折叠**） | `crawl_gd_economy.py`（**周更 CI**；多页列表；仅入库含 GDP 的期次） | 省统计局「经济运行简况」：不变价 GDP/三产增加值；附规上工业、社消零、固投、**房开投资**、CPI；**居民人均可支配收入**（全省/城镇/农村 · 名义/实际同比）；**GDP/房开/收入 ≠ 城市挂牌/网签均价**；月度无 GDP 则跳过 |
...
gd_economy.csv               → gdEconomy.ts          → 广东经济运行（GDP/可支配收入）
