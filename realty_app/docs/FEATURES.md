...
| 入口 | 能力摘要 | 功能 ID |
|------|----------|---------|
| 首页 | 城市、核心市场脉搏、快捷入口、推荐房源 | F-ENTRY-02 |
| 找房 | 房源 / 小区 / 学校三类检索与详情闭环 | F-LIST / F-COMM / F-SCH |
| 地图 | 地图找房、行政区与社区分析 | F-MAP-* |
| 行情 | 网签、70 城、供需、宏观、趋势和数据工具 | F-MKT-01 |
| 我的 | 本地收藏、浏览历史、外观、数据状态、更新与关于 | F-MINE-01 / F-SET-* |

完整验收见 [APP_SHELL_ACCEPTANCE.md](./APP_SHELL_ACCEPTANCE.md)。

## 2. 首页（Dashboard）

| 能力 | 说明 | 功能 ID |
|------|------|---------|
| 多入口 | 城市 + 搜索 + 核心快捷入口 | F-ENTRY-02 |
...
| 行情 | 网签、70 城、供需、宏观、趋势与工具统一入口 |
| 我的 | 本地收藏和浏览历史（仅本机）+ 主题、关于、检查更新 / OTA |
| 网签子页 | 深广政府网签明细 |
...
最后更新：2026-08-18（v1.122.0 五栏 IA、行情聚合与本地资料库）
