...
**期望**：列表可点进学校/小区（有 id 时）；城市过滤正确；**广深教育事业概览**有官方 CSV 时展示，深圳用「普通中小学」口径且不显示假小学/初中。  
**不期望**：深圳数据出现在广州 Tab 未开跨城时；珠海伪造教育统计。  
**自动化**：matrix；school_* / hospital smoke；unit school* / `educationOverview`  
**手工**：深圳城总览/学校页见教育卡；珠海无该卡。
...
**期望**：数字来自对应 CSV；空则隐藏或空态；**NBS 多期销售面积同比可并列展示**（累计口径，非成交均价）；广州库存卡显示新鲜度（>3 天滞后文案）；日更 CI merge 库存、月更 CI merge NBS。  
**自动化**：unit `lprHistory` / `nbsRealEstate`（≥3 期） / `gzNewHouseInventory` / `gzInventoryFreshness`  
**手工**：广州城总览库存卡日期旁见「截至…」；宏观卡见「1—4 · 1—5 · 1—6」同比串。
