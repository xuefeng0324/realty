...
**期望**：数字来自对应 CSV；空则隐藏或空态；官方宏观卡按 **全国 / 广东** 分组（`macro-kicker`），脚注短句、城乡人口进折叠；**NBS 多期销售面积/销售额/投资/到位资金同比可并列展示（默认折叠）**；**NBS 房屋施工/新开工/竣工面积（官方表，多期同比）**；**NBS 销售额÷面积派生全国合同均价（多期）+ 粗算可售月数（多期；明示非城市均价/去化）**；**NBS 全国固定资产投资基本情况（累计额+民间/制造/设备/三产同比，多期默认折叠；≠房价）**；**NBS 居民收入和消费（含居住消费，季/半年/年报；≠房价）**；**NBS 月度 CPI（含居住/租赁房房租同比；房租≠房价）**；**70城当前城市 Top% 细进度条**；广州库存卡新鲜度与分区可售占比；**保障房同年棚改竣工脚注（不同口径，不并排 KPI）**；深圳计划入市；广州住房发展计划；**广州保障房已筹建/已竣工清单汇总**；**广州保障房任务量目标进度（筹集/竣工）**；**深圳保障房建设筹集/基本建成项目表**；**深圳/广州公积金年报（深圳含上年对照）+ 珠海公积金动态（含同月末对照）+ 广东全省年报（默认折叠）**；**广东房地产市场运行简况（多期默认折叠；含省统计局上半年/累月）**；**广东固定资产投资简况（同比多期默认折叠）**；**广东建筑业生产运行（产值多期默认折叠）**；**广东经济运行（GDP/房开/收入/年报人口·城镇化多期默认折叠）**；**广州居住用地成交**（仅 R2/居住，土地价款≠房价）；**深圳居住用地已成交**（列表起始价≠成交总价≠房价）；**珠海安居工程进展**（年内累计开工/竣工，非成交）；涨跌色涨红跌绿；网签/70城/城市标签须同步 store（不可等异步 cities）。  
**自动化**：unit `gzHousingPlan` / `gzAffordableTargets` / `szProvidentAnnual` / `gzProvidentAnnual` / `zhProvidentDynamics` / `gdProvidentAnnual` / `gdRealEstateBrief` / `gdFaInvestment` / `gdConstruction` / `gzLandDeals` / `nbsRealEstate` / `nbsFaInvestment` / `nbsIncome` / `nbsCpi` / `adversarialColorCitySupply` / city-sync adversarial  
