...
**期望**：数字来自对应 CSV；空则隐藏或空态；**NBS 多期销售面积同比可并列展示**；广州库存卡新鲜度；深圳计划入市；广州住房发展计划；**广州保障房已筹建/已竣工清单汇总**；**深圳保障房建设筹集/基本建成项目表**；**广州居住用地成交**（仅 R2/居住，土地价款≠房价）；**深圳居住用地已成交**（列表起始价≠成交总价≠房价）；**珠海安居工程进展**（年内累计开工/竣工，非成交）；涨跌色涨红跌绿；网签/70城/城市标签须同步 store（不可等异步 cities）。  
**自动化**：unit `gzHousingPlan` / `gzLandDeals` / `adversarialColorCitySupply` / city-sync adversarial  
