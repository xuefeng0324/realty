...
**期望**：数字来自对应 CSV；空则隐藏或空态；**NBS 多期销售面积同比可并列展示**（累计口径，非成交均价；≥5 期含 1—2…1—6）；广州库存卡显示新鲜度（>3 天滞后文案）；日更 CI merge 库存、月更 CI merge NBS；**深圳城显示「计划入市」季度供应卡**；**广州城显示《住房发展年度计划》卡**（批准预售面积/用地/保障房筹建，年更计划口径）；涨跌色统一 **涨红跌绿**。  
**自动化**：unit `lprHistory` / `nbsRealEstate` / `gzNewHouseInventory` / `szPlannedSupply` / `gzHousingPlan` / `adversarialColorCitySupply`  
**手工**：广州城见住房发展计划卡（2026 vs 2025）；深圳城见计划入市 Q 卡。
...
| 2026-07-26 | F-DASH-11 | 广州住房发展年度计划 + 珠海探针结论 | 1.121.55 | 公文附件解析；珠海仍不可抓 |
