...
v1.122.0 品牌色固定为翡翠绿主色 + 科技蓝信息色；新增页面必须使用统一 token 和共享页面壳，不得在页面内复制一套浅/深色常量。主题壳 API 返回 Promise 时也必须显式吸收 rejection，H5 smoke 不得出现 `pageerror`。
...
最后更新：2026-08-18（统一品牌 token、共享页面壳与主题 Promise rejection 门禁）
