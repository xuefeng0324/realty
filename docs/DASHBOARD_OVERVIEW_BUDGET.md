...
| ID | 描述 | 收益 | 优先级 |
|----|------|------|--------|
| T-001 | **nbs-macro 卡重构**：20 KPI 太满。拆成「主 4 KPI（开发投资 / 新房销售 / 面积 / 待售）+ 「查看完整管线」按钮 → 跳转 macro-region 子页（或 nbs-macro 子页）」 | -150 行 + 单卡 ≤ 6 KB | 高 |
| T-002 | **MacroKpiCell 全量铺开**：dashboard 还有 53 处 stats70-cell，其中 macro-* 区约 30 处。本轮只改了 nbs-macro 卡的 16 KPI；剩余 30+ KPI + 3 个 macro 子页全部替换为组件 | -300 行 + 模板统一 | 高 |
| T-003 | **3 个 macro 子页用 MacroKpiCell 重写**：macro-region / macro-industry / macro-trade 内 53 张卡 × 4 KPI = 200+ 节点可压缩 | -600 行跨文件 | 中 |
| T-004 | **非宏卡瘦身**：dashboard 还有 114 张非宏卡（房价三轴 + 学校 + 通勤 + 地图 tab），平均 100 行/张；可考虑抽到 `<XxxCard>` 子组件 + 模板复用 | -3000 行（理论上限） | 低（架构大改） |
| T-005 | **加「市场风向」Compact 6 格**（预算 §1 规划）：当 overview 仍有空 slot 时再加 | +1 张宏卡 + 紧凑 | 低（与「弄短首页」指令冲突，**待用户拍板**） |
| T-006 | **nbs-macro 多期序列再缩**：当前默认收起，展开后 6 个 v-for 块（均价/可售月/面积/销售额/投资/资金）。可进一步缩到 3 块（仅均价/可售月/销售额） | -12 行（仅展开区） | 低 |
