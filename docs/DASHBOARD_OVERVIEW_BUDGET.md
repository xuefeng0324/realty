...
| 维度 | 限 | 取值逻辑 |
|------|----|----------|
| `overview` tab 内「独立 macro-card」最大数 | **≤ 4**（v1.121.136 收紧） | 贝壳首页同屏可见 2–3 张；本地 3–4 屏到 footer |
| `overview` tab 内「所有 .card 的 `<view>` 直系子元素」最大数 | **≤ 12** | 含 4 张 macro-card + 搜索/筛选/三轴/footer 上限 |
| `dashboard.vue` 总行数 | **≤ 11 500**（v1.121.144 收紧；远期 ≤ 10 000） | v1.121.144 当前 10 975；先把每新增卡移到子页后逐步缩 |
...
| `pages/macro-rates/*.vue` | LPR / MLF / 逆回购 / 国债 / Shibor / 回购 FR/FDR（6 张卡 + 38 KPI，v1.121.135 完成） | 已建（25 → 390 行） |
...
| T-008 | **macro-rates 子页从骨架升级为 6 张利率卡（LPR/MLF/逆回购/Shibor/国债/回购 FR/FDR）+ 38 KPI** | +365 行（子页增加） | ✅ **v1.121.135 完成**（25 → 390 行） |
| T-009 | **macro-rates 多期序列展示**：当前仅 latest + 1 期 delta；可补 6/12 个月 mini sparkline | ~+80 行 | **待用户拍板** |
| T-010 | **macro-rates 二级 Tab 分组**：Shibor 短端/长端；国债 1y/3-5y/10y+ 分组 | -50 行 + 可读性 + | **待用户拍板** |
| T-011 | **LPR 深度页 `pages/lpr-deep/`**：利差时间线 / 调息周期 / 累计降息 / 1 年前对比 | +200 行（新页） | **待用户拍板** |
...
| T-012 | **dashboard.vue 真删 15 张宏卡 HTML + 对应 refs/imports**（MLF/OMO/Shibor/国债/FR-FDR/央行金融统计/广东社融/8 张外汇） | -1,206 行 | ✅ **v1.121.136 完成**（15,392 → 14,186） |
| T-013 | **新增 `pages/data-tools/data-tools.vue`**：把派生卡（学校/教育/通勤/挂牌结构/区情画像/特征溢价/挂牌标签/地铁规划）分组迁移 | -2,000 行（理论上限） | **下一轮**（T-013） |
| T-014 | **价格 tab 折叠**：把 `data-tab="overview,price"` 下所有非核心派生卡包进 `<view v-if="priceDerivedExpanded">` 折叠 | -0 行（仅 UI） | **下一轮**（T-014） |

### 本轮实际收益（v1.121.130 → v1.121.144）
...
| v1.121.131 | 15,404 | 1 | + nbs-macro 多期精简 + MacroKpiCell 铺 nbs-macro 16 KPI |
| v1.121.132 | 15,404 | 1 | + 3 个 macro 子页 52 KPI 改 MacroKpiCell（子页合计 -245） |
| v1.121.133 | **15,392** | 1 | + nbs-macro 多期序列精简 6 → 4（实测 -12） |
| v1.121.134 | 15,392 | 1 | + macro-fx 子页 8 张汇市卡 47 KPI（23 → 453 行） |
| v1.121.135 | 15,392 | 1 | + macro-rates 子页 6 张利率卡 38 KPI（25 → 390 行） |
| v1.121.136 | **14,186** | 1 | **真删 dashboard 内 15 张宏卡**（-1,206 行 / -7.8%）；预算收紧到 ≤ 4 / ≤ 14.5k |
| v1.121.137 | **14,239** | 1 | + 数据工具页 (data-tools.vue) + 派生卡折叠块入口；不增加 dashboard 长度（折叠块仍存在） |
| v1.121.142 Batch 1-3 | **12,024** | 1 | **真删 dashboard 内 14 张派生卡**（-2,215 行 / -15.5%）；设置入口「数据工具」独立页生效 |
